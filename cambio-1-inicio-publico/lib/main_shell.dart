...
/// Botón/menú de usuario (arriba a la derecha del encabezado).
/// - Sin sesión: muestra un botón "Iniciar Sesión" que lleva al login.
/// - Con sesión: muestra el avatar + menú (Mi Perfil / Cerrar Sesión).
class _UserMenu extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final user = AuthService().currentUser;

    // Sin sesión activa: solo se ofrece el acceso a iniciar sesión. Todavía
    // no hay nada que "cerrar" ni un perfil que mostrar.
    if (user == null) {
      return TextButton.icon(
        onPressed: () => Navigator.pushNamed(context, '/login'),
        icon: const Icon(Icons.login, size: 18, color: AppColors.emerald700),
        label: const Text(
          'Iniciar Sesión',
          style: TextStyle(
    ...
    return PopupMenuButton<String>(
      ...
      onSelected: (v) async {
        if (v == 'perfil') {
          ...
          if (context.mounted) {
            // Tras cerrar sesión se vuelve al mismo shell, ahora sin
            // sesión: sigue viendo Inicio público, pero como "Viajero" y
            // con el botón "Iniciar Sesión" de nuevo en el encabezado.
